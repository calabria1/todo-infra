...
# =========================================
# S3 Bucket para Artefatos (já existe)
# =========================================
module "artifacts" {
  source = "../../modules/s3"
...
module "lambda_tasks" {
  ...
  function_name = "${local.name}-tarefas-handler"
  runtime       = "python3.11"
...
module "api" {
  ...
  routes = [
    ...
    { method = "GET",    path = "/tarefas" },
    { method = "GET",    path = "/tarefas/{id}" },
    { method = "PUT",    path = "/tarefas/{id}" },
    { method = "DELETE", path = "/tarefas/{id}" }
  ]
